...
module.exports = {
    ...
    async execute(message, arguements, botClient) {
        const DamagedUser = message.mentions?.users?.first()?.id || arguements[0] 
        const Damage = parseInt(arguements[1])
        const Reason = arguements.slice(2).join(" ") || "No Reason Provided"

        const OldData = JSON.parse(DataHandler.getUser(message.guild.id, DamagedUser).damage)
        ...
        const UserDamageJsonReturned = JSON.parse(DataHandler.addDamage(message.guild.id, DamagedUser, NewJson).damage)
        ...
        if (TotalDamageCalculated >= 25) {
            ...
            await message.channel.send(`User <@${DamagedUser}> has been banned for reaching taking more than 25 damage, total: ${TotalDamageCalculated}`)
            await KickedMember.kick(Reason)
            await KickedMember.send(`<@${DamagedUser}>, You have been banned for reaching taking more than 25 damage, total: ${TotalDamageCalculated}`)
        } else {
            await message.channel.send(`User <@${DamagedUser}> has taken ${Damage} damage for ${Reason} and now has a total of ${TotalDamageCalculated}`)
        }
    }
}